...
export default function Dashboard() {
  ...
  const aiContext = useMemo(
    () => ({
      ...
      inventory: {
        ...
        highlights: topLowStock.slice(0, 3).map(item => ({
          name: item.name,
  ...
  // 🔄 Keep supplier draft in sync with latest topLowStock
  useEffect(() => {
                  ...
                  >
                    ...
                    {scheduledExport && nextScheduledRun ? (
                      <p style={{ margin: 0, fontSize: 13, color: '#0F172A' }}>
                        ...
                        {nextScheduledRun.toLocaleDateString()} with{' '}
                        {scheduledExport.skuCount} SKUs.
                      </p>
                    ) : (
                      <p style={{ margin: 0, fontSize: 13, color: '#475569' }}>
                      ...
                      <div style={{ margin: 0, fontSize: 13, color: '#0F172A' }}>
                        ...
                        >
                          {supplierDraft.items.slice(0, 3).map(item => (
                            <li key={item.id}>
                              {item.name} · order {item.suggestedQuantity} (on hand{' '}
                              {item.stockCount})
                            </li>
                          ))}
