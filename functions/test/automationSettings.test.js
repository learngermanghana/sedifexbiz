...
assert.equal(isSmsAutomationEnabledForEvent(perRule, 'booking.confirmed'), false, 'unsupported SMS events must remain disabled')
assert.equal(perRule.channels['booking.confirmed'].sms, false, 'parser must reject unsupported SMS toggles')
