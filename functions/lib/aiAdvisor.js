...
exports.generateAiAdvice = void 0;
const functions = __importStar(require("firebase-functions/v1"));
const params_1 = require("firebase-functions/params");
const firestore_1 = require("firebase-admin/firestore");
const firestore_2 = require("./firestore");
const OPENAI_API_KEY = (0, params_1.defineString)('OPENAI_API_KEY');
const MODEL_NAME = 'gpt-4o-mini';
const MAX_CONTEXT_CHARS = 12000;
// ---------- Helpers: coercion / formatting ----------
function coerceStoreId(data, context) {
    const explicitStoreId = typeof data.storeId === 'string' && data.storeId.trim() ? data.storeId.trim() : null;
    if (explicitStoreId)
        return explicitStoreId;
    const tokenStoreId = typeof context.auth?.token?.storeId === 'string' && context.auth.token.storeId.trim()
        ? context.auth.token.storeId
        : null;
    if (!tokenStoreId) {
        throw new functions.https.HttpsError('invalid-argument', 'storeId is required either in the request or as a custom claim.');
    }
    return tokenStoreId;
}
function normalizeJsonContext(raw) {
...
function pickWorkspaceData(raw) {
    if (!raw)
        return null;
    const fields = [
        'company',
...
async function buildSalesSummary(storeId) {
    const { start, end } = getTodayRange();
    const snapshot = await firestore_2.defaultDb
        .collection('sales')
    ...
    snapshot.forEach(docSnap => {
        ...
        for (const item of items) {
            ...
            const nameCandidate = typeof item?.name === 'string' ? item.name.trim() : 'Unknown product';
            const name = nameCandidate || 'Unknown product';
            const idCandidate = typeof item?.productId === 'string' ? item.productId : name;
...
async function buildTrendSummary(storeId, days) {
...
async function buildExpenseSummary(storeId, days) {
    const { start, end } = getLastNDaysRange(days);
    const snapshot = await firestore_2.defaultDb
        .collection('expenses')
        .where('storeId', '==', storeId)
...
async function fetchRecentCloseouts(storeId) {
    const closeouts = await firestore_2.defaultDb
    ...
    return closeouts.docs.map(docSnap => {
        ...
        return {
            ...
            closedByName: typeof data.closedBy?.displayName === 'string'
...
async function fetchProductCounts(storeId) {
    const baseQuery = firestore_2.defaultDb.collection('products').where('storeId', '==', storeId);
...
async function fetchRecentActivity(storeId) {
    const snapshot = await firestore_2.defaultDb
        .collection('activity')
        .where('storeId', '==', storeId)
        .orderBy('createdAt', 'desc')
...
async function fetchGoalProgress(storeId, monthSales) {
    // Assumes a document in storeGoals with id == storeId
    const goalSnap = await firestore_2.defaultDb.collection('storeGoals').doc(storeId).get();
    if (!goalSnap.exists) {
        return {
            target: null,
            period: null,
...
async function buildContext(storeId, userContext) {
    const todayRange = getTodayRange();
    const monthStart = new Date();
    monthStart.setDate(1);
    monthStart.setHours(0, 0, 0, 0);
    const monthStartTs = firestore_1.Timestamp.fromDate(monthStart);
    const [workspaceSnap, storeSnap, salesToday, sales7d, salesPrev7d, expenses7d, closeouts, productCounts, activity, monthSalesSnap,] = await Promise.all([
        firestore_2.defaultDb.collection('workspaces').doc(storeId).get(),
        firestore_2.defaultDb.collection('stores').doc(storeId).get(),
    ...
    if ('docs' in monthSalesSnap && Array.isArray(monthSalesSnap.docs)) {
        ;
        monthSalesSnap.docs.forEach((docSnap) => {
            const data = docSnap.data();
    ...
    return {
        ...
        productCounts,
        recentActivity: Array.isArray(activity) ? activity : activity?.error,
    };
}
// ---------- OpenAI call ----------
async function callOpenAI(question, contextJson) {
    const apiKey = OPENAI_API_KEY.value();
    if (!apiKey) {
        throw new functions.https.HttpsError('failed-precondition', 'OPENAI_API_KEY is not configured for this project.');
    }
    ...
    if (!response.ok) {
        ...
        throw new functions.https.HttpsError('internal', `OpenAI error ${response.status}: ${errorText.substring(0, 400)}`);
    }
