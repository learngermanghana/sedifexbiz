...
function appointment(data: RecordMap) {
  const date = bookingLifecycleDate(data)
  const time = displayTime(bookingLifecycleTime(data))
...
function rootEventId(storeId: string, bookingId: string, eventId: string) {
  return encodeURIComponent(`${storeId}|${bookingId}|${eventId}`)
}

function eventRef(storeId: string, bookingId: string, eventId: string) {
  return defaultDb.collection('bookingLifecycleSmsEvents').doc(rootEventId(storeId, bookingId, eventId))
}

function queueRef(storeId: string, bookingId: string, eventId: string) {
  return defaultDb.collection('bookingLifecycleSmsQueue').doc(rootEventId(storeId, bookingId, eventId))
}
...
async function enqueueLifecycleEvent(
  storeId: string,
  bookingId: string,
  stage: LifecycleSmsStage,
  eventKey: string,
  ...
  await defaultDb.runTransaction(async tx => {
    ...
    tx.set(event, base)
    tx.set(queue, { ...base, dueDateKey: today })
  })
}
...
export const queueBookingLifecycleSmsOnWrite = functions.firestore
  ...
  .onWrite(async (change, context) => {
    ...
    const tasks = transitions
      ...
      .map(event => enqueueLifecycleEvent(storeId, bookingId, event.stage, event.eventKey, after))
...
async function loadQueueItem(doc: FirebaseFirestore.QueryDocumentSnapshot): Promise<QueueItem | null> {
...
async function loadBooking(item: QueueItem): Promise<BookingContext | null> {
  ...
  const [storeSnapshot, bookingSnapshot] = await Promise.all([storeRef.get(), ref.get()])
...
function eligible(item: QueueItem, booking: BookingContext) {
  ...
  const currentSchedule = bookingLifecycleScheduleKey(booking.data)

  if (item.stage === 'booking_received') return !cancelled && !completed && status !== 'confirmed'
  if (item.stage === 'booking_confirmed') return status === 'confirmed' && (!item.scheduleKey || item.scheduleKey === currentSchedule)
  if (item.stage === 'booking_rescheduled') return !cancelled && !completed && (!item.scheduleKey || item.scheduleKey === currentSchedule)
  return cancelled
}

async function markTerminal(item: QueueItem, status: 'disabled' | 'stale' | 'booking_missing') {
  await Promise.allSettled([
    item.eventRef.set({ status, updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true }),
    item.ref.delete(),
  ])
}
...
async function staleSending(item: QueueItem) {
  ...
  if (lockedAt && Date.now() - lockedAt.getTime() < STALE_MINUTES * 60_000) return true
...
async function processQueueDoc(doc: FirebaseFirestore.QueryDocumentSnapshot, rates: SmsRateTable) {
  const item = await loadQueueItem(doc)
  if (!item) return 'invalid'
  if (await staleSending(item)) {
    const booking = await loadBooking(item)
    if (booking && norm(item.data.status) === 'sending') {
  ...
  try {
    ...
    await Promise.allSettled([
      item.ref.set({
        status: 'unknown',
        dueDateKey: DISABLED_DATE,
        providerAcceptedAt: admin.firestore.FieldValue.serverTimestamp(),
        lastError: note,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      }, { merge: true }),
      item.eventRef.set({
