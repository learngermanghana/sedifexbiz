...
function stateLabel(task: PortalTask) {
  ...
  if (task.clientState === 'submitted') return 'Sent to event team'
  if (task.clientState === 'changes_requested') return 'Needs your attention'
  return 'To do'
}
...
export function eventClientPortalHtml(data: EventClientPortalPageData, token: string) {
  ...
  const taskHtml = data.tasks.length
    ? data.tasks.map(task => {
      ...
      const needsStart = task.status === 'todo'
      const changesRequested = task.clientState === 'changes_requested'
      const actionHtml = verified
        ? `<div class="task-finished"><strong>✓ Done</strong><p>${task.verifiedAt ? `Confirmed by your event team · ${escapeHtml(formatDateTime(task.verifiedAt))}` : 'Confirmed by your event team.'}</p></div>`
        : submitted
          ? '<div class="waiting"><strong>✓ Sent to event team</strong><p>Waiting for confirmation. The event team will confirm it or send it back with a note.</p></div>'
          : `<div class="task-completion"><label class="task-check"><input id="done-${escapeHtml(task.id)}" data-task-complete="${escapeHtml(task.id)}" type="checkbox" onchange="toggleTaskDone('${escapeHtml(task.id)}')"><span>${changesRequested ? 'I have fixed this' : 'I have done this'}</span></label><div id="finish-${escapeHtml(task.id)}" class="task-finish" hidden><label for="note-${escapeHtml(task.id)}">Anything you want to tell us? <small>Optional</small></label><textarea id="note-${escapeHtml(task.id)}" data-task-id="${escapeHtml(task.id)}" placeholder="Example: Guest list confirmed."></textarea><button id="submit-${escapeHtml(task.id)}" type="button" onclick="completeTask('${escapeHtml(task.id)}',${needsStart ? 'true' : 'false'})">Send to event team</button></div></div>`
      return `<article class="task">
        ...
        ${task.clientSubmissionNote ? `<div class="client-note"><strong>Your last message:</strong> ${escapeHtml(task.clientSubmissionNote)}</div>` : ''}
        ${actionHtml}
      </article>`
    }).join('')
    : '<div class="empty">You do not have any tasks from the event team right now.</div>'
  ...
  const programApprovalLabel = data.program.preparingRevision
    ? 'Update in progress'
    : data.program.status !== 'approved'
      ? 'Not yet published'
      : data.program.clientApproved
        ? 'Approved'
        : data.program.requireClientApproval
          ? 'Awaiting your approval'
          : 'Published'
  const programApprovalArea = data.program.clientApproved
    ? `<div class="program-approved"><strong>You approved this program</strong><p>${escapeHtml(data.program.clientApprovedBy || 'Client')}${data.program.clientApprovedAt ? ` · ${escapeHtml(formatDateTime(data.program.clientApprovedAt))}` : ''}</p></div>`
    : data.program.canApprove
      ? `<div class="program-approval"><strong>Is this program okay?</strong><p class="muted">Check the program above. If everything is correct, approve it. If something should change, tell the event team below.</p><div class="program-actions"><button id="program-approve" type="button" onclick="approveProgram()">Approve program</button><span id="program-approval-status" class="program-status" role="status"></span></div></div>`
      : data.program.status === 'approved' && !data.program.requireClientApproval && !data.program.preparingRevision
        ? '<div class="program-published-note"><strong>Published by your event team</strong><p>No approval is required. You can still ask for a change below.</p></div>'
        : ''
  const programRequestArea = pendingProgramRequest
    ...
    : data.program.items.length && data.program.canRequestChanges
      ...
      : data.program.items.length && data.program.preparingRevision
        ? `<div class="request-pending"><strong>The event team is preparing an update</strong><p>The last published program remains visible while the event team prepares the next version.</p></div>`
        : ''
  const programBadgeBackground = data.program.preparingRevision || (data.program.requireClientApproval && !data.program.clientApproved)
    ? '#fff7ed'
    : data.program.status === 'approved' ? '#ecfdf5' : '#f1f5f9'
  const programBadgeColor = data.program.preparingRevision || (data.program.requireClientApproval && !data.program.clientApproved)
    ? '#9a3412'
    : data.program.status === 'approved' ? '#166534' : '#475569'
  const publishedMeta = data.program.status === 'approved'
    ? `Published revision ${data.program.revision}${data.program.publishedAt ? ` · ${escapeHtml(formatDateTime(data.program.publishedAt))}` : ''}`
...
<section class="card"><div class="program-head"><div><h2>Your event program</h2><p class="muted">Check the published program here. If something should change, send the event team a request instead of editing it directly.</p></div><div><span class="program-badge">${programApprovalLabel}</span><p class="muted" style="text-align:right;margin:6px 0 0">${publishedMeta}</p></div></div><div class="program-list">${programItemsHtml}</div>${programApprovalArea}${programRequestArea}${programRequestHistory ? `<div style="margin-top:14px"><strong style="font-size:.78rem">Recent requests</strong>${programRequestHistory}</div>` : ''}</section>
</section>
<section id="portal-panel-tasks" class="tab-panel" data-tab-panel="tasks" role="tabpanel" aria-labelledby="portal-tab-tasks" hidden>
<section class="card"><h2>Things you need to do</h2><p class="muted">When you finish a task, tick <strong>I have done this</strong>. You can add a message if needed, then send it to the event team for confirmation.</p><div class="tasks">${taskHtml}</div></section>
</section>
...
function hasDirtyDraft(){return briefDirty||programRequestDirty||Array.prototype.some.call(document.querySelectorAll('textarea[data-task-id]'),function(area){return Boolean(area.value&&area.value.trim())})||Array.prototype.some.call(document.querySelectorAll('input[data-task-complete]'),function(input){return Boolean(input.checked)})}
...
async function approveProgram(){var button=document.getElementById('program-approve');var status=document.getElementById('program-approval-status');if(button)button.disabled=true;if(status){status.textContent='Approving…';status.className='program-status'}var ok=await postPortalAction({action:'approve_program',revision:${data.program.revision},fingerprint:${JSON.stringify(data.program.fingerprint)}});if(ok)location.reload();else{if(status){status.textContent='Could not approve the program. Review the latest version and try again.';status.className='program-status error'}if(button)button.disabled=false}}
...
function toggleTaskDone(taskId){var checkbox=document.getElementById('done-'+taskId);var details=document.getElementById('finish-'+taskId);if(details)details.hidden=!(checkbox&&checkbox.checked)}
async function startTask(taskId){var ok=await postPortalAction({action:'start',taskId:taskId,note:''});if(ok)location.reload()}
async function completeTask(taskId,needsStart){var checkbox=document.getElementById('done-'+taskId);var button=document.getElementById('submit-'+taskId);if(!checkbox||!checkbox.checked)return;if(button)button.disabled=true;if(needsStart){var started=await postPortalAction({action:'start',taskId:taskId,note:''});if(!started){if(button)button.disabled=false;return}}var note=document.getElementById('note-'+taskId);var ok=await postPortalAction({action:'submit',taskId:taskId,note:note?note.value:''});if(ok){try{sessionStorage.removeItem(draftKey(taskId))}catch(_error){}location.reload()}else if(button)button.disabled=false}
