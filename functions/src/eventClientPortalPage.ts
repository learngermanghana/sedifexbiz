...
export function eventClientPortalHtml(data: EventClientPortalPageData, token: string) {
  ...
  const taskHtml = data.tasks.length
    ...
    : '<div class="empty">There are no checklist tasks shared with you yet. You can still update your event brief and review the program above.</div>'
...
<section class="card program-card"><div class="program-head"><div><h2>Program for review</h2><p class="muted">This document is read-only in the client portal. If something should change, send a request instead of editing the approved program directly.</p></div><div><span class="program-badge">${programApprovalLabel}</span><p class="muted" style="text-align:right;margin:6px 0 0">Revision ${data.program.revision}${data.program.approvedBy ? ` · ${escapeHtml(data.program.approvedBy)}` : ''}</p></div></div><div class="program-list">${programItemsHtml}</div>${pendingProgramRequest ? `<div class="request-pending"><strong>Change request awaiting the event team</strong><p>${escapeHtml(pendingProgramRequest.message)}</p></div>` : data.program.items.length ? `<div class="program-request"><strong>Request a program change</strong><p class="muted">Describe exactly what you would like changed. The current program stays unchanged until the event team accepts your request and opens a new revision.</p><textarea id="program-change-request" data-program-change-request placeholder="e.g. Please move the couple entrance after the welcome speech."></textarea><div class="program-actions"><button id="program-change-submit" type="button" onclick="submitProgramChange()">Send change request</button><span id="program-change-status" class="program-status" role="status"></span></div></div>` : ''}${programRequestHistory ? `<div style="margin-top:14px"><strong style="font-size:.78rem">Recent requests</strong>${programRequestHistory}</div>` : ''}</section>
